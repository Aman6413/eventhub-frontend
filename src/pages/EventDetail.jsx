...
import { fetchEventById, registerForEvent } from "../store/thunks/eventDetailThunk"; // ✅ NEW
...
const EventDetail = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            const res = await dispatch(fetchEventById({ eventId: id, token: user.token }));
            if (!res.error) {
                dispatch(getMyRegistrations(user.token));
                const regRes = await dispatch(fetchEventRegistrations({ id: id, token: user.token })); // ✅ NEW
    ...
    const handleRegister = async () => {
        const res = await dispatch(registerForEvent({ token: user.token, eventId: id }));
        ...
        <div className="max-w-5xl mx-auto mt-10 p-6 bg-white shadow rounded">
            <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="flex-1">
                    <h1 className="text-3xl font-bold text-blue-700 mb-4">{selectedEvent.title}</h1>

                    <p className="mb-2 text-gray-700">
                        <strong>Date:</strong> {moment(selectedEvent.date, "DD-MM-YYYY").format("DD MMMM YYYY")}
                    </p>
                    <p className="mb-2 text-gray-700">
                    ...
                    <p className="mb-2 text-gray-700">
                        <strong>Coordinator Contact No:</strong> {selectedEvent.contactNumber}
                    </p>
                    <p className="mb-2 text-gray-700">
            ...
            {user?.role === "admin" && eventRegistrations.length > 0 && (
                <div className="mt-10">
                    <h2 className="text-2xl font-semibold text-blue-800 mb-5 border-b pb-2">Registered Users</h2>
