...
const AdminPage = () => {
    ...
    const handleCreateOrUpdate = async () => {
        if (formData.date) formData.date = moment(formData.date).format("DD-MM-YYYY");
        if (formData.time) formData.time = moment(formData.time, "HH:mm").format("h:mm A");
            ...
            {showForm && (
                <div className="mt-4 bg-gray-100 p-4 rounded">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <input type="text" name="title" value={formData.title} onChange={handleInputChange} placeholder="Title" className="p-2 border rounded" />
                        <input type="text" name="description" value={formData.description} onChange={handleInputChange} placeholder="Description" className="p-2 border rounded" />
                        ...
                        <input type="text" name="location" value={formData.location} onChange={handleInputChange} placeholder="Location" className="p-2 border rounded" />
                        <input type="text" name="contactNumber" value={formData.contactNumber} onChange={handleInputChange} placeholder="Contact Number" className="p-2 border rounded" />
                        ...
                        <div>
                            <label className="block mb-1 font-semibold">Image (Max 500 KB)</label>
                    ...
                    <div className="mt-4">
                        <Button onClick={handleCreateOrUpdate} disabled={!isFormValid} className={!isFormValid ? "bg-gray-300 cursor-not-allowed" : ""}>
                ...
                {list.map((event) => (
                    <div key={event._id} className="border p-4 rounded shadow">
                        ...
                        <div className="mt-3 flex gap-3">
                            ...
                            <Button onClick={() => handleDelete(event._id)} className="bg-red-500">Delete</Button>
                            <Button onClick={() => navigate(`/events/${event._id}`)} className="bg-green-600">Open</Button>
                        </div>
                    </div>
                ))}
            </div>
